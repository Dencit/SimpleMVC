...
class wxpay extends apiBase {
    ...
    function __construct(){
        ...
        self::$MS=model::model('wxpay');//装载对应model时，自动连接数据库
...
/*
        ...
        if($USR){
            ...
            $uid2openid=self::$WX->convert_to_openid($access_token,array("userid"=>$corp_userid));
...
*/
        ...
        $input->SetNotify_url(self::$PA->HTTP_BASE."/Api/wxpay/result/uid-".$this->uid);
    ...
    function result(){
        ...
        ////检查单号 预防重复回调
        $RECHARGE_HAD=$MS->rowSelect(self::$TB->RECHARGE,'uid,out_trade_no','out_trade_no/'.$xmlArr['out_trade_no'].',uid/'.$uid);
        ...
        //判断是否在编会员
        $USR_INFO=$MS->rowSelect(self::$TB->USR_INFO,"`join`",array('uid'=>$uid));
        ...
        if($is_join=='0'){
            ...
            ////修改用户补贴记号
            $subsidySign=$MS->rowUpdate(self::$TB->USR_INFO, array('subsidy_type' => $subsidy_type), "uid/" . $this->uid);
            ...
            ////开始写充值记录
            $rechargeSet=$MS->rowInsert(self::$TB->RECHARGE,$rechData);
        ...
        if($is_join=='1'){
            ...
            //var_dump($where);//exit;//
            $recharged=$MS->rowSelect(self::$TB->RECHARGE,'*',$where,'time desc');
            ...
            if(!$recharged&&$balance_add>='22'){
                ...
                if($balance_add>='22') {
                    ...
                    ////修改用户补贴记号
                    $subsidySign=$MS->rowUpdate(self::$TB->USR_INFO, array('subsidy_type' => $subsidy_type), "uid/" . $uid);
