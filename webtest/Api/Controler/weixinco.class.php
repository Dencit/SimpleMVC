...
class weixinco extends apiBase {
    ...
    function __construct(){
        ...
        //账号
        self::$acc_arr["name"] = "[NAME]";//可以访问此api的帐号
        //账号对应密匙
        self::$passwd_arr["name"] = "[PASS_WORD]";//可以访问此api的帐号密码
    ...
    function init(){
        ...
        /* access_token */
        $access_token = self::$RED->get(PREFIX."access_token");
        $access_token_ttl = self::$RED->ttl(PREFIX."access_token");//读取expire 设置的过期时间

        if(!$access_token || $access_token_ttl < 300 || self::$refresh=='refresh'){
            $data = self::$WX->access_token();
            if(!$data){
                //发送警报给管理员
            }else{
            ...
            if(!isset($data->access_token) || !isset($data->expires_in)){
                ...
                self::$RED->set(PREFIX."access_token", $data->access_token);
                self::$RED->expire(PREFIX."access_token", ($data->expires_in - 10));
        ...
        /* jsapi_ticket */
        $jsapi_ticket = self::$RED->get(PREFIX."jsapi_ticket");
        $jsapi_ticket_ttl = self::$RED->ttl(PREFIX."jsapi_ticket");

        if(!$jsapi_ticket || $jsapi_ticket_ttl < 300 || self::$refresh=='refresh'){
            $data = self::$WX->jsapi_ticket($access_token);
            if(!$data){
                //发送警报给管理员
            }else{
                $data = json_decode($data);
            ...
            if(!isset($data->ticket) || !isset($data->expires_in)){
                ...
                self::$RED->set(PREFIX."jsapi_ticket", $data->ticket);
                self::$RED->expire(PREFIX."jsapi_ticket", ($data->expires_in - 10));
        ...
        /* api_ticket */
        $api_ticket = self::$RED->get(PREFIX."api_ticket");
        $api_ticket_ttl = self::$RED->ttl(PREFIX."api_ticket");

        if(!$api_ticket || $api_ticket_ttl < 300 || self::$refresh=='refresh'){
            $data = self::$WX->api_ticket($access_token);
            if(!$data){
                //发送警报给管理员
            }else{
                $data = json_decode($data);
            ...
            if(!isset($data->ticket) || !isset($data->expires_in)){
                ...
                self::$RED->set(PREFIX."api_ticket", $data->ticket);
                self::$RED->expire(PREFIX."api_ticket", ($data->expires_in - 10));
                echo " [ api_ticket OK ] \n";
            }
        }else{
    ...
    function login(){
        ...
        if($debug){ $redirect_uri = self::$PA->API_URL.'/authCode/&debug=1'; }
        else{ $redirect_uri = self::$PA->API_URL.'/authCode/'; }
        ...
        while ( self::$RED->exists($state) ){
            $state = PREFIX.'loginAuth_'.md5($acc.$final_redirect_uri.$user_type.TIME.rand(0,100000));
        }
        self::$RED->set($state,$final_redirect_uri);
        self::$RED->expire($state,300);
        //最终跳转地址
        setcookie(PREFIX."loginAuth_back",$final_redirect_uri);
        //\\

        self::$WX->get_auth_code($redirect_uri,$user_type,$state);
    }
    ...
    function oauth(){
        ...
        if($debug){
            $redirect_uri = urlencode(self::$PA->API_URL.'/userId/&debug=1');
        }else{
            $redirect_uri = urlencode(self::$PA->API_URL.'/userId/');
        }
        ...
        self::$RED->set($state,$final_redirect_uri);
        self::$RED->expire($state,60);
        setcookie(PREFIX."OAuth_back",$final_redirect_uri);

        //var_dump($redirect_uri);exit;//

        self::$WX->get_code($scope,$state,$redirect_uri);//成功则去到 openid page
    ...
    static function userId(){
        ...
        $back = urldecode( self::$RED->get($state) );
        ...
        $corp_data = self::$WX->get_usersign_co($access_token,$code);
        ...
        if( isset($corp_data->UserId) ){
            //企业成员用户标识
            $open_Data = self::$WX->convert_to_openid($access_token,array("userid"=>$corp_data->UserId) );
    ...
    static function access_token(){
        ...
        $access_token = self::$RED->get(PREFIX."access_token");
        if($access_token){
            $expires_in = self::$RED->ttl(PREFIX."access_token") - 300;
    ...
    static function api_ticket(){
        ...
        $api_ticket = self::$RED->get(PREFIX."api_ticket");
        if($api_ticket){
            $expires_in = self::$RED->ttl(PREFIX."api_ticket") - 300;
            $expires_in = ($expires_in <= 0) ? 5 : $expires_in;
            $rsp->errcode = 1;
            $rsp->errmsg = "success";
    ...
    static function js_api_ticket(){
        ...
        $jsapi_ticket = self::$RED->get(PREFIX."jsapi_ticket");
        if($jsapi_ticket){
            $expires_in = self::$RED->ttl(PREFIX."jsapi_ticket") - 300;
            $expires_in = ($expires_in <= 0) ? 5 : $expires_in;
            $rsp->errcode = 1;
            $rsp->errmsg = "success";
