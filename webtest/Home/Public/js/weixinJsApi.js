...
////副方法
    ...
    noPlayShare:function(data,func){
        ...
        option.allShare.imgUrl='http://wx.host.com/Home/Public/images/icon.jpg';// 分享小图
    ...
    playedShare:function(data,func){
        ...
        option.allShare.link= THIS.local('host')+"Home/?/weixin/index/sid-"+data.sid+"/";// 分享链接
        option.allShare.imgUrl='http://wx.host.com/Home/Public/images/icon.jpg';// 分享小图
        option.allShare.success=function(){};
        option.allShare.cancel=function(){};
        THIS.set(option);
...
account.jsApi='http://wx.host.com/weixin/jsApi.php?'+"url="+wxja.local('url',1)+"&t="+new Date().getTime();
wxja.init(account);
