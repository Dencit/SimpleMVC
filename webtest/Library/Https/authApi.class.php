...
class authApi{
    ...
    function __construct(){
     ...
     static function init($PATH=null){
        ...
        self::$OAUTH2_URI=$PATH->ACCOUNT;
        self::$ACCESS_TOKEN=$PATH->PASSWD;
        self::$OAUTH2_URI=$PATH->OAUTH2_URI;
        self::$ACCESS_TOKEN=$PATH->ACCESS_TOKEN;
...
//有限授权
     static function usrAuth($redirect_uri = '',$scope = 'snsapi_userinfo'){
        ...
        $url=self::$OAUTH2_URI."/?acc=".self::$ACCOUNT."&time=".$time."&sign=".md5($sign)."&scope=".$scope."&state=".urlencode($redirect_uri);
        header("location:".$url);
        exit;
    }
...
//获取全局access_token
     static function globeAccessToken(){
        ...
        $url = self::$ACCESS_TOKEN."/?acc=".self::$ACCOUNT."&time=".$time."&sign=".md5($sign);
        $data = self::http($url);
        if($data){
